...
const longestString = (arr) => {
  ...
  },[]);//[] we put [] to push the rsult as array beaause whene we use indexOf it's take array 
  let ree =  arr.indexOf(result) ;
  return ree ;
};
...
Write a function named findHappiness that takes in an array of strings and returns an
 array containing only the strings from the input array that contain ":)".
...
const findHappiness = (arr) => {
  ...
  let c ;
  for (let i = 0; i < arr.length; i++) {
    let re = arr[i].includes(':)') ;
    re ? array.push( arr[i]) :c ++ ;
  }
  return array ;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    let ete_1 = arr[i].substr(1,3) ;
    let ete_2 = arr[i].substr(6,3) ;
    let ete_3 = arr[i].substr(10,14) ;
    let res =  ete_1+ete_2+ete_3 ;
    array.push(res) ;
  }
  return array ;
...
const onlyOddChars = (str) => {
  let c ;
  let split =  str.split('') ;
  let result = split.filter((ele,index)=>{
    return index % 2 === 1 ? ele : c++ ;
  });
...
const allHappy = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let re = arr[i].includes(':)') ;
    re ? array.push( arr[i]) :check=false ;
  }
  return check ;
};
