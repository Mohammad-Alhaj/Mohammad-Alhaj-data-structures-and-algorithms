...
uses either filter, map, or reduce to count the amount of times the integer is present in the array of arrays.
...
const count = (target, input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    input[i].reduce((acc,ele)=>{

      ele===target ? count++ : null ;
      return count ;
    },0);
...
Write a function that, given an array of integer arrays as input, calculates the total 
...
const totalSum = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    let result =  input[i].reduce((acc,ele)=>{
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    let result = input[i].filter(ele=>{
      return typeof(ele) === 'number' && Number.isInteger( (ele/5)) ?' Math.pow(2, 10)' : null ;
    });
    for (let i = 0; i < result.length; i++) {
      array.push( Math.pow(2, result[i])) ;
    }
    array_2.push(array);
    array=[];
  }
...
let findMaleAndFemale = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (data[i].gender==='female'||data[i].gender==='male') {

      i<data.length-1 ? sum =sum+`${data[i].name} and `: sum =sum+`${data[i].name}`;
    }
...
Write a function named findShortest that, given the Star Wars data 
from Challenge 6, uses any combination of filter, map and reduce to return the name
 of the character who is the shortest in height.
------------------------------------------------------------------------------------------------ */
...
let findShortest = (data) => {
 ...
 for (let i = 0; i < data.length; i++) {
  ~~data[i].height=== result ? theName=data[i].name:null ;
    }
 return theName ;
};
