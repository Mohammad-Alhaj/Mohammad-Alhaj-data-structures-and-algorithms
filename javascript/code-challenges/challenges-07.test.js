...
const wordsToCharList = (arr) => {
  ...
  // could solve this problem with split method bult in javascript ...
};
...
const listFoods = (recipe) => {
  let result = [];
  let summay = recipe.ingredients ;
  // console.log(summay[1]);
  for (let i = 0; i < summay.length; i++) {
    let numIndex = summay[i].indexOf(' ',3) ;
    result.push(summay[i].slice(numIndex+1)) ;
