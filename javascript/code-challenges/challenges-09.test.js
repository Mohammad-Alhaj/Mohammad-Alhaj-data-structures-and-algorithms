...
for (const key in arr) {
    ...
    let x = houses.push( arr[key].house) ;
  // I used for in method becauee the key return 1,2,3,4,5,6, not the key (houses,...) could use anothe for th deal with this problem
...
const hasChildrenValues = (arr, character) => {
  ...
  for (let key in arr) {
    ...
    let x = arr[key].name .includes(character);
...
describe('Testing challenge 1', () => {
  ...
  test.skip('It should handle negatives and return the maximum number found', () => {
...
describe('Testing challenge 6', () => {
  ...
  test('It should return false to characters who do not have children', () => {
    expect(hasChildrenValues(characters, 'Sansa')).toBeTruthy();
  });
});
